...
// @Router /customer/healthProfessional/add-nurse-review [post]
func AddNurseReview(c *fiber.Ctx) error {
	...
	callback := func(sessCtx mongo.SessionContext) (interface{}, error) {
		...
		if err1 == mongo.ErrNoDocuments {
			...
			if service.Nurse != nil {
				newTotalReviews = service.Nurse.Review.TotalReviews + 1
				newAverageRating = ((service.Nurse.Review.AvgRating * float64(service.Nurse.Review.TotalReviews)) +
					review.Rating) / float64(newTotalReviews)
			}

			update := bson.M{
				"$set": bson.M{
					"nurse.review.totalReviews": newTotalReviews,
					"nurse.review.avgRating":    newAverageRating,
				},
			}
			...
			if service.Nurse != nil {
				newTotalReviews = service.Nurse.Review.TotalReviews
				newAverageRating = ((service.Nurse.Review.AvgRating * float64(service.Nurse.Review.TotalReviews)) - existingReviews.Rating + review.Rating) / float64(newTotalReviews)
			}
