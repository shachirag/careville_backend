...
// @Router /customer/add-review [post]
func AddReview(c *fiber.Ctx) error {
	...
	callback := func(sessCtx mongo.SessionContext) (interface{}, error) {
		...
		if err1 == mongo.ErrNoDocuments {
			...
			newTotalReviews = service.TotalReviews + 1
			newAverageRating = ((service.AvgRating * float64(service.TotalReviews)) +
			...
			newTotalReviews = service.TotalReviews
			newAverageRating = ((service.AvgRating * float64(service.TotalReviews)) - existingReviews.Rating + review.Rating) / float64(newTotalReviews)
			update = bson.M{
				"$set": bson.M{
					"totalReviews": newTotalReviews,
					"avgRating":    newAverageRating,
