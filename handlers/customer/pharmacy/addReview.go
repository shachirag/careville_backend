...
// @Router /customer/healthFacility/add-pharmacy-review [post]
func AddPharmacyReview(c *fiber.Ctx) error {
	...
	callback := func(sessCtx mongo.SessionContext) (interface{}, error) {
		...
		if err1 == mongo.ErrNoDocuments {
			...
			if service.Pharmacy != nil {
				newTotalReviews = service.Pharmacy.Review.TotalReviews + 1
				newAverageRating = ((service.Pharmacy.Review.AvgRating * float64(service.Pharmacy.Review.TotalReviews)) +
					review.Rating) / float64(newTotalReviews)
			}

			update := bson.M{
				"$set": bson.M{
					"pharmacy.review.totalReviews": newTotalReviews,
					"pharmacy.review.avgRating":    newAverageRating,
				},
			}
			...
			if service.Pharmacy != nil {
				newTotalReviews = service.Pharmacy.Review.TotalReviews
				newAverageRating = ((service.Pharmacy.Review.AvgRating * float64(service.Pharmacy.Review.TotalReviews)) - existingReviews.Rating + review.Rating) / float64(newTotalReviews)
			}
