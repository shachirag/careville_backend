...
// @Produce json
// @Success 200 {object} hospitals.GetEmergencyHospitalResDto
// @Router /customer/emergency/get-emergency-hospitals [get]
func GetEmergencyHospitals(c *fiber.Ctx) error {
	...
	if latParam != "" && longParam != "" {
		...
		if err != nil {
			return c.Status(fiber.StatusBadRequest).JSON(hospitals.GetEmergencyHospitalResDto{
				Status:  false,
	...
	filter := bson.M{
		...
		"hospClinic.information.isEmergencyAvailable": true,
	}
	...
	projection := bson.M{
		...
		"_id":                          1,
		"hospClinic.information.isEmergencyAvailable": 1,
	...
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(hospitals.GetEmergencyHospitalResDto{
			Status:  false,
	...
	var hospitalData []hospitals.GetEmergencyHospitalsRes
	for cursor.Next(ctx) {
		...
		if hospital.HospClinic != nil {
			hospitalData = append(hospitalData, hospitals.GetEmergencyHospitalsRes{
				Id:    hospital.Id,
				...
				IsEmergencyAvailable: hospital.HospClinic.Information.IsEmergencyAvailable,
				Fees:                 0,
			})
		}
	}
	...
	if len(hospitalData) == 0 {
		return c.Status(fiber.StatusOK).JSON(hospitals.GetEmergencyHospitalResDto{
			Status:  false,
