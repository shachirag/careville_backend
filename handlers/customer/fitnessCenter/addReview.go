...
// @Router /customer/healthFacility/add-fitnessCenter-review [post]
func AddFitnessCenterReview(c *fiber.Ctx) error {
	...
	callback := func(sessCtx mongo.SessionContext) (interface{}, error) {
		...
		if err1 == mongo.ErrNoDocuments {
			...
			if service.FitnessCenter != nil {
				newTotalReviews = service.FitnessCenter.Review.TotalReviews + 1
				newAverageRating = ((service.FitnessCenter.Review.AvgRating * float64(service.FitnessCenter.Review.TotalReviews)) +
					review.Rating) / float64(newTotalReviews)
			}

			update := bson.M{
				"$set": bson.M{
					"fitnessCenter.review.totalReviews": newTotalReviews,
					"fitnessCenter.review.avgRating":    newAverageRating,
				},
			}
			...
			if service.FitnessCenter != nil {
				newTotalReviews = service.FitnessCenter.Review.TotalReviews
				newAverageRating = ((service.FitnessCenter.Review.AvgRating * float64(service.FitnessCenter.Review.TotalReviews)) - existingReviews.Rating + review.Rating) / float64(newTotalReviews)
			}
