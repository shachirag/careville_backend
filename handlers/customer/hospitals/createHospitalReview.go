...
// @Router /customer/healthFacility/add-hospital-review [post]
	...
	callback := func(sessCtx mongo.SessionContext) (interface{}, error) {
		...
		if err1 == mongo.ErrNoDocuments {
			...
			if service.HospClinic != nil {
				newTotalReviews = service.HospClinic.Review.TotalReviews + 1
				newAverageRating = ((service.HospClinic.Review.AvgRating * float64(service.HospClinic.Review.TotalReviews)) +
					review.Rating) / float64(newTotalReviews)
			}

			update := bson.M{
				"$set": bson.M{
					"hospClinic.review.totalReviews": newTotalReviews,
					"hospClinic.review.avgRating":    newAverageRating,
				},
			}
			...
			if service.HospClinic != nil {
				newTotalReviews = service.HospClinic.Review.TotalReviews
				newAverageRating = ((service.HospClinic.Review.AvgRating * float64(service.HospClinic.Review.TotalReviews)) - existingReviews.Rating + review.Rating) / float64(newTotalReviews)
			}
