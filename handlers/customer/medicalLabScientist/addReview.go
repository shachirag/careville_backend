...
// @Router /customer/healthProfessional/add-medicalLabScientist-review [post]
func AddMedicalLabScientistReview(c *fiber.Ctx) error {
	...
	callback := func(sessCtx mongo.SessionContext) (interface{}, error) {
		...
		if err1 == mongo.ErrNoDocuments {
			...
			if service.MedicalLabScientist != nil {
				newTotalReviews = service.MedicalLabScientist.Review.TotalReviews + 1
				newAverageRating = ((service.MedicalLabScientist.Review.AvgRating * float64(service.MedicalLabScientist.Review.TotalReviews)) +
					review.Rating) / float64(newTotalReviews)
			}

			update := bson.M{
				"$set": bson.M{
					"medicalLabScientist.review.totalReviews": newTotalReviews,
					"medicalLabScientist.review.avgRating":    newAverageRating,
				},
			}
			...
			if service.MedicalLabScientist != nil {
				newTotalReviews = service.MedicalLabScientist.Review.TotalReviews
				newAverageRating = ((service.MedicalLabScientist.Review.AvgRating * float64(service.MedicalLabScientist.Review.TotalReviews)) - existingReviews.Rating + review.Rating) / float64(newTotalReviews)
			}
