...
// @Produce json
// @Success 200 {object} medicalLabScientist.GetMedicalLabScientistAppointmentsPaginationRes
// @Router /customer/healthProfessional/appointment/medicalLabScientist-approved-appointments [get]
func FetchApprovedMedicalLabScientistAppointmentsWithPagination(c *fiber.Ctx) error {
	...
	if err != nil {
		return c.Status(fiber.StatusBadRequest).JSON(medicalLabScientist.GetMedicalLabScientistAppointmentsPaginationRes{
			Status:  false,
	...
	if err != nil {
		if err == mongo.ErrNoDocuments {
			return c.Status(fiber.StatusNotFound).JSON(medicalLabScientist.GetMedicalLabScientistAppointmentsPaginationRes{
				Status:  false,
				Message: "appointment not found",
			})
		}
		return c.Status(fiber.StatusInternalServerError).JSON(medicalLabScientist.GetMedicalLabScientistAppointmentsPaginationRes{
			Status:  false,
	...
	response := medicalLabScientist.MedicalLabScientistAppointmentsPaginationResponse{
		Total:          0,
		PerPage:        limit,
		CurrentPage:    page,
		TotalPages:     0,
		AppointmentRes: []medicalLabScientist.GetMedicalLabScientistAppointmentsRes{},
	}
	...
	finalResponse := medicalLabScientist.GetMedicalLabScientistAppointmentsPaginationRes{
		Status:  true,
