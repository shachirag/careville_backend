...
// @Router /customer/healthProfessional/add-doctor-review [post]
func AddDoctorReview(c *fiber.Ctx) error {
	...
	callback := func(sessCtx mongo.SessionContext) (interface{}, error) {
		...
		if err1 == mongo.ErrNoDocuments {
			...
			if service.Doctor != nil {
				newTotalReviews = service.Doctor.Review.TotalReviews + 1
				newAverageRating = ((service.Doctor.Review.AvgRating * float64(service.Doctor.Review.TotalReviews)) +
					review.Rating) / float64(newTotalReviews)
			}

			update := bson.M{
				"$set": bson.M{
					"doctor.review.totalReviews": newTotalReviews,
					"doctor.review.avgRating":    newAverageRating,
				},
			}
			...
			if service.Doctor != nil {
				newTotalReviews = service.Doctor.Review.TotalReviews
				newAverageRating = ((service.Doctor.Review.AvgRating * float64(service.Doctor.Review.TotalReviews)) - existingReviews.Rating + review.Rating) / float64(newTotalReviews)
			}
