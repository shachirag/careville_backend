...
func AddPhysiotherapistAppointment(c *fiber.Ctx) error {
	...
	overlapFilter := bson.M{
		...
		"physiotherapist.service.id": physiotherapistServiceDataServiceObjID,
		"physiotherapist.appointmentDetails.from": bson.M{"$lte": toDate},
		"physiotherapist.appointmentDetails.to":   bson.M{"$gte": fromDate},
	}
	...
	if count > 0 {
		return c.Status(fiber.StatusBadRequest).JSON(physiotherapist.PhysiotherapistAppointmentResDto{
			Status:  false,
			Message: "You have already created a booking for this service with the physiotherapist.",
		})
	}
	...
	callback := func(sessCtx mongo.SessionContext) (interface{}, error) {
		...
		update := bson.M{
			"$push": bson.M{
				"physiotherapist.upcommingEvents": bson.M{
					"id":        appointment.Id,
					"startTime": fromDate,
					"endTime":   toDate,
				},
