...
// @Router /customer/healthProfessional/add-physiotherapist-review [post]
func AddPhysiotherapistReview(c *fiber.Ctx) error {
	...
	callback := func(sessCtx mongo.SessionContext) (interface{}, error) {
		...
		if err1 == mongo.ErrNoDocuments {
			...
			if service.Physiotherapist != nil {
				newTotalReviews = service.Physiotherapist.Review.TotalReviews + 1
				newAverageRating = ((service.Physiotherapist.Review.AvgRating * float64(service.Physiotherapist.Review.TotalReviews)) +
					review.Rating) / float64(newTotalReviews)
			}

			update := bson.M{
				"$set": bson.M{
					"physiotherapist.review.totalReviews": newTotalReviews,
					"physiotherapist.review.avgRating":    newAverageRating,
				},
			}
			...
			if service.Physiotherapist != nil {
				newTotalReviews = service.Physiotherapist.Review.TotalReviews
				newAverageRating = ((service.Physiotherapist.Review.AvgRating * float64(service.Physiotherapist.Review.TotalReviews)) - existingReviews.Rating + review.Rating) / float64(newTotalReviews)
			}
