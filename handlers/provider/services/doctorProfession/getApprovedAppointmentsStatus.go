...
// @Produce json
// @Success 200 {object} services.GetDoctorAppointmentsPaginationRes
// @Router /provider/services/appointment/doctor-approved-appointments [get]
func FetchDoctorApprovedAppointmentsWithPagination(c *fiber.Ctx) error {
	...
	if err != nil {
		if err == mongo.ErrNoDocuments {
			return c.Status(fiber.StatusNotFound).JSON(services.GetDoctorAppointmentsPaginationRes{
				Status:  false,
				Message: "appointment not found",
			})
		}
		return c.Status(fiber.StatusInternalServerError).JSON(services.GetDoctorAppointmentsPaginationRes{
			Status:  false,
	...
	response := services.DoctorAppointmentsPaginationResponse{
		Total:          0,
		PerPage:        limit,
		CurrentPage:    page,
		TotalPages:     0,
		AppointmentRes: []services.GetDoctorAppointmentsRes{},
	}
	...
	for cursor.Next(ctx) {
		...
		if appointment.Doctor != nil {
			fromDate = appointment.Doctor.AppointmentDetails.From
			toDate = appointment.Doctor.AppointmentDetails.To
		}
	...
	finalResponse := services.GetDoctorAppointmentsPaginationRes{
		Status:  true,
