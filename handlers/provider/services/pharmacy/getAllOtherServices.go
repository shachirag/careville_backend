...
func GetOtherServices(c *fiber.Ctx) error {
	...
	if err != nil {
		if err == mongo.ErrNoDocuments {
			return c.Status(fiber.StatusNotFound).JSON(services.GetPharmacyOtherServicesResDto{
				Status:  false,
				Message: "other service not found",
			})
		}
		return c.Status(fiber.StatusInternalServerError).JSON(services.GetPharmacyOtherServicesResDto{
			Status:  false,
			Message: "Failed to fetch other service from MongoDB: " + err.Error(),
		})
	}

	serviceData := make([]services.PharmacyOtherServiceRes, 0)
	if service.Pharmacy != nil && len(service.Pharmacy.AdditionalServices) > 0 {
		for _, service := range service.Pharmacy.AdditionalServices {
			serviceData = append(serviceData, services.PharmacyOtherServiceRes{
				Id:          service.Id,
				Name:        service.Name,
