...
func UpdateOtherServiceInfo(c *fiber.Ctx) error {
	...
	var (
		...
		data        services.UpdatePharmacyOtherServiceReqDto
	...
	if err != nil {
		return c.Status(400).JSON(services.UpdatePharmacyOtherServiceResDto{
			Status:  false,
	...
	update := bson.M{
		"$set": bson.M{
			"pharmacy.additionalServices.$.name":        data.Name,
			"pharmacy.additionalServices.$.information": data.Information,
