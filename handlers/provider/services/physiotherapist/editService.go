...
// @Param provider body services.UpdatePhysiotherapistServiceReqDto true "Update data of service"
// @Produce json
// @Success 200 {object} services.UpdatePhysiotherapistServiceResDto
// @Router /provider/services/update-physiotherapist-service/{serviceId} [put]
func UpdatePhysiotherapistServiceInfo(c *fiber.Ctx) error {

	var (
		serviceColl = database.GetCollection("service")
		data        services.UpdatePhysiotherapistServiceReqDto
	...
	if err != nil {
		return c.Status(500).JSON(services.UpdatePhysiotherapistServiceResDto{
			Status:  false,
			Message: err.Error(),
		})
	}
	...
	update := bson.M{
		"$set": bson.M{
			"physiotherapist.serviceAndSchedule.$.serviceFees": data.ServiceFees,
			"physiotherapist.serviceAndSchedule.$.name":        data.Name,
			"physiotherapist.serviceAndSchedule.$.slots":       bson.A{},
	...
	return c.Status(fiber.StatusOK).JSON(services.UpdatePhysiotherapistServiceResDto{
		Status:  true,
